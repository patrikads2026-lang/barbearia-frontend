...
export default function Painel() {
  ...
  async function adicionarBarbeiro() {
    ...
    await fetch(`${API}/api/barbeiros`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ nome: novoBarbeiro.trim() }),
    });
    setNovoBarbeiro("");
    buscarBarbeiros();
  }
  ...
  async function salvarEdicao(nomeAntigo) {
    ...
    await fetch(`${API}/api/barbeiros`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ nomeAntigo, nomeNovo: nomeEditado.trim() }),
    });
    setEditando(null);
    setNomeEditado("");
    buscarBarbeiros();
    ...
    <div className="painel-wrap">
      <div className="painel-container">
        ...
        {aba === "barbeiros" && (
          <div>
            <div className="painel-bloquear-form">
              <input className="painel-input" placeholder="Nome do novo barbeiro" value={novoBarbeiro} onChange={e => setNovoBarbeiro(e.target.value)} onKeyDown={e => e.key === "Enter" && adicionarBarbeiro()} />
            ...
            <div className="painel-lista">
              {barbeiros.length === 0 ? <p className="painel-vazio">Nenhum barbeiro cadastrado.</p> : barbeiros.map(b => (
                <div key={b} className="painel-item">
                  {editando === b ? (
                    <>
                      <input className="painel-input" value={nomeEditado} onChange={e => setNomeEditado(e.target.value)} onKeyDown={e => e.key === "Enter" && salvarEdicao(b)} style={{ flex: 1, marginBottom: 0 }} />
